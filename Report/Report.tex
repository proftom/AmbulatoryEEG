...
The throughput tends assymtopically to the maximum of 236.686 kbps, and min to max throughput is above 7%. From 

From the preliminary work, it has been shown the CSR1010 is capable of running at 9 packets per \ac{CE} stable. 

As the CI the chance for RF interference increases (e.g. a CRC error). The \ac{BLE} specifications states that once a transmission error occurs during transmission, the \ac{CE} should stop and wait until the next \ac{CE}. Hence, there exists a trade off between the \ac{CI} and risk of premature link termination. This is demonstrated in the 

Some devices only allow so many packets per \ac{CE}, constraining the system further, and in such cases, having a smaller \ac{CE} is of benefit. The iPhone was observed to allow, on average only 6 packets per \ac{CE} and limit the \ac{CI} to a minimum of 30ms. From the Apple developer guidelines, it is possible to reduce this interval to 20ms. 
...
A test harness was created to investigate the relationship between throughput and connection interval and compare it against the theoretical maximum. 
...
This seems to produce a profiles where at sleep time, the current is very close 5 $\mu$As.
