...
To measure the power usage of the system a 10 ohm shunt resistor was put between the board's $V_{ss}$ and ground. Scope leads where then attached to measure the voltage, which due to ohm's law, will be measured at 10 times the current. 

Firsly, the power usage of the just the radio was performed (the ADCs were disconnected).  The data sheet claims the maximum peak current used during transmission or receiving is approximately 16mA at 3V. 
...
\begin{figure}[H]
	...
	\end{center}
	\caption{Connection events - connection interval set at 1 second with a slave latency of 0}
	\label{fig:ce}
\end{figure}
...
\begin{figure}[H]
	...
	\end{center}
	\caption{Connection events - connection interval set at 1 second}
	\label{fig:ceclose}
\end{figure}

To investigate the "base" power consumption the device requires, that is the power consumption required to maintain a connection indefinately. Taking the voltage data in the figures above, captured using an oscilloscope, it is possible to accurately estimate the area under the profile using the trapezium rule. This was done using the matlab function $cumtrapz$. For 3.3 volts, this area is found to be approximately 11.76. This area represents the current consumption in milli-amp-hours for a single conneciton event. To convert this to the current consumption in units milli-amp-hour not per connection event, the number is multiplied by the number of connection events (set by the connection interval) within an hour period, and divded through by the number of milliseconds in an hour to convert the time units (the x-axis in Figure~\ref{fig:cesim} is in milliseconds).
...
This number is the "base" amount, which must be paid every time data is sent. To achieve a higher economical value of energy for a given throughput, this fixed cost needs to be amortised as much as possible. 

\begin{figure}[H]
	\begin{center}
		\includegraphics[width = 0.9\textwidth]{cesim}
	\end{center}
	\caption{Connection events - connection interval set at 1 second}
	\label{fig:cesim}
\end{figure}
...
\begin{displaymath}
\min_{x}  \frac{1250\times x}{676} - \floor{ \frac{1250\times x}{676}} 
\end{displaymath}
