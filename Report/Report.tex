...
There already exists much literature on \ac{EEG} technology and archetypal  signals measured (such as \cite{Teplan2002}), hence only a brief overview is presented here. \ac{EEG} signals can be detected on the scalp with a typical range between 0.3-60Hz 

 The report is not concerned with the analouge front end for filtering and capturing signals, howver digital conversion through sampling and providing sufficient resolution is of importance. 
...
\begin{figure}[htb]
	...
	\end{center}
	\caption{Application - 6 channels configured, with data being shown }
...
Another way to improve throughput include compression techniques, for which there exists many information resources on lossless and lossy compressing \ac{EEG} data \cite{Antoniol1997} \cite{Wongsawat2006}. A problem with using such a technique, is that if data is spread over multiple notifications, any data loss of a single notification, may result in the loss of a larger chunk of information. 
