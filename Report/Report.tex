...
In the sole pursuit of maximizing throughput, the idea try and pack as many notifications into a \ac{CE}l. Unfortunately . We can create a remainder profile 

\begin{displaymath}
\min_{x}  \frac{1250\times x}{676} - \floor{ \frac{1250\times x}{676}} 
\end{displaymath}

While some devices only allow so many packets per \ac{CE}
...
As the CI the chance for RF interference increases (e.g. a CRC error). The \ac{BLE} specifications states that once a transmission error occurs during transmission, the \ac{CE} should stop and wait until the next \ac{CE}. Hence, there exists a trade off between the \ac{CI} and risk of premature link termination. 
