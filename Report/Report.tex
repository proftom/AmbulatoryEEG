...
\end{quote}[sic]
 ...
 In the first paragraph the engineer is treating the full 27 bytes in the data payload as the application data, and is believe this number has come from

\begin{displaymath}
\frac{1000000 \mu s}{676 \mu s} \times 27 < 320 kbps
\end{displaymath}

320 to 300 is a generous approximation, it is believe that the engineer actually rounded a smaller number, 305, which was calculated by including an unecessary 32 bit \ac{MIC}, which is only used for when encryption is enabled:

\begin{displaymath}
\frac{1000000 \mu s}{708 \mu s} \times 27 \approx 305 kbps \approx 300 kbps
\end{displaymath}

Throughout literature on \ac{BLE}, this is a recurring common mistake.  
Different sources quote different numbers. From examining sources, it is believed the varation 

In the second paragraph the engineer is highlighting some important facets of \ac{BLE} devices, and that is although one end of the link is determined by the lower of the two device's thorughputs. The engineer cites smart phones as such devices that restrict the link capability. This was seen in the nRF8001 - the iPhone 4S could not handle a connection interval less than 30ms. When asked how to show the math behind these numbers, they replied
...
The \ac{HCI} is the component between the host and controller layers shown in Figure~\ref{fig:ble_arch}. The engineer is saying that 

the full throughput as defined in the \ac{BLE} specification. \ac{CSR} supplied a development kit consisting of  provided a development board (CNS10004V5D) and \ac{USB} dongle. The CSR1010 is part of \ac{CSR} 's new $\mu$Energy product line, for which \ac{CSR} have developed their own compiler and \ac{IDE} known as xIDE. \


For initial throughput testing, an example health thermometer application was used. 
...
Ultimately, the decision is to use an off-board \ac{ADC} module, the AD7997. The prototype will interface with two of these over an I2C bus, to provide 16 channels.
...
\begin{figure}[htb]
	\begin{center}
		\includegraphics[width = 0.9\textwidth]{boardv1}
	\end{center}
...
Alot quote 708 as minimum, but it's 676 and confirmed
